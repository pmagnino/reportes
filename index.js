...
// 1. FORZAR HUSO HORARIO ARGENTINA A NIVEL SISTEMA
...
// CONFIGURACIÓN DE RUTAS E ESTÁTICOS
app.get('/', (req, res) => {
    res.sendFile(path.join(__dirname, 'public', 'reporte_index.html'));
});

app.get('/index.html', (req, res) => {
    res.redirect('/reporte_index.html');
});

app.use(express.static(path.join(__dirname, 'public')));

// Limpieza de parámetros para iPhone (evita el error ":1")
const cleanParam = (p) => {
    if (!p) return '';
    return p.toString().split(':')[0].trim();
};

// ==========================================
// UTILS: FECHA REAL ARGENTINA (GMT-3)
// ==========================================
app.get('/api/util/fecha-hoy', (req, res) => {
    // Esta es la forma más segura de obtener la fecha YYYY-MM-DD en Argentina
...
app.get('/api/facturas', async (req, res) => {
    let sucursal = cleanParam(req.query.sucursal);
    let desde = cleanParam(req.query.desde);
    let hasta = cleanParam(req.query.hasta);
    let articulo = cleanParam(req.query.articulo);
    let medioPago = cleanParam(req.query.medioPago);
    ...
    try {
        ...
        if (facturas.length > 0) {
            ...
            facturas.forEach(f => {
                f.items = itemsRes.recordset.filter(i => i.IdRouter === f.IdRouter).map(i => ({ cod: i.CODITM, cant: i.CANTIDAD1, pre: i.PRECIO, obs: i.OBSERVACIONES }));
            });
        }
    ...
    try {
        ...
        const result = await pool.request()
            ...
            .query(`
                ...
                Ventas AS (
                    SELECT A.IdRouter, (CASE WHEN A.CODCMP IN ('CA', 'CB') THEN -A.TOTAL ELSE A.TOTAL END) as MONTO,
                        CASE WHEN DATEPART(HOUR, A.timestamp) >= 9 AND DATEPART(HOUR, A.timestamp) < 13 THEN 'MAÑANA (09-13hs)' 
                             WHEN DATEPART(HOUR, A.timestamp) >= 13 AND DATEPART(HOUR, A.timestamp) < 17 THEN 'TARDE 1 (13-17hs)' 
                             ELSE 'TARDE 2 (17-22hs)' END AS F_ASIG
                    FROM dbo.QRMVS A 
                ...
                FROM MasterFranjas M LEFT JOIN Ventas V ON M.F_NOM = V.F_ASIG
...
app.get('/api/reporte/stock-fisico', async (req, res) => {
    ...
    try {
        ...
        if (articulo !== "") q += ` AND S.CODITM LIKE '%' + @art + '%'`;
        const result = await pool.request().input('suc', sql.Int, sucursal).input('art', sql.VarChar, articulo).query(q + " ORDER BY S.CODITM");
        res.json(result.recordset);
    } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.get('/api/reporte/stock-valorizado', async (req, res) => {
    ...
    try {
        const pool = await poolMainPromise;
        const result = await pool.request().input('suc', sql.Int, sucursal).query(`
...
app.get('/api/reporte/logistica', async (req, res) => {
    ...
    try {
        ...
        if (articulo !== "") q += ` AND B.CODITM LIKE '%' + @art + '%'`;
        const result = await pool.request().input('suc', sql.Int, sucursal).input('desde', sql.VarChar, desde).input('hasta', sql.VarChar, hasta).input('art', sql.VarChar, articulo).query(q + " ORDER BY A.FECHA DESC");
        const movs = {};
...
app.get('/api/reporte/ranking', async (req, res) => {
    ...
    try {
        const pool = await poolMainPromise;
        const result = await pool.request().input('suc', sql.Int, sucursal).input('desde', sql.VarChar, desde).input('hasta', sql.VarChar, hasta).query(`
...
// --- 10. COMPARATIVO (CORRECCIÓN FINAL DE FECHA LUNES) ---
app.get('/api/reporte/comparativo-ventas', async (req, res) => {
    let sucursal = cleanParam(req.query.sucursal);
    let fechaRef = cleanParam(req.query.fechaRef);

    // Obtenemos la hora actual en Argentina de forma segura
    const hoyArgentina = new Date(new Intl.DateTimeFormat('en-US', {timeZone: 'America/Argentina/Buenos_Aires'}).format(new Date()));
    
    // Si viene fechaRef (ej: 2026-01-15), le clavamos las 12 del mediodía para que no se mueva el día
    let targetDate = fechaRef ? new Date(fechaRef + 'T12:00:00') : hoyArgentina;
    ...
    try {
        ...
        const result = await pool.request()
            ...
            .query(`
                ...
                VentasBase AS (
                    SELECT CAST(M.FECHA AS DATE) as F, SUM(CASE WHEN M.CODCMP IN ('CA', 'CB') THEN -M.TOTAL ELSE M.TOTAL END) as MontoDia,
                    ISNULL((SELECT SUM(CASE WHEN M2.CODCMP IN ('CA', 'CB') THEN -I.CANTIDAD1 ELSE I.CANTIDAD1 END) FROM dbo.QRMVS M2 INNER JOIN dbo.QRLINEASITEMS I ON M2.IdRouter = I.IdRouter WHERE CAST(M2.FECHA AS DATE) = CAST(M.FECHA AS DATE) AND CAST(M2.CODSUC AS VARCHAR) LIKE '%' + @suc AND M2.CODCMP IN ('FA', 'FB', 'CA', 'CB') AND M2.ANULADO = 0 AND M2.IDCOMPROBANTE > 0 AND I.CODITM NOT LIKE 'DC%'), 0) as UnidadesDia
                    FROM dbo.QRMVS M WHERE CAST(M.CODSUC AS VARCHAR) LIKE '%' + @suc AND M.CODCMP IN ('FA', 'FB', 'CA', 'CB') AND M.ANULADO = 0 AND M.IDCOMPROBANTE > 0 GROUP BY CAST(M.FECHA AS DATE)
                )
                SELECT FORMAT(C.FechaAct, 'dd/MM') as FechaLabel, UPPER(LEFT(DATENAME(WEEKDAY, C.FechaAct), 3)) as DiaSemana, FORMAT(C.FechaSemAnt, 'dd/MM') as LabelSem, FORMAT(C.FechaMesAnt, 'dd/MM') as LabelMes, '$ ' + FORMAT(ISNULL(V1.MontoDia, 0), '#,0.00') as MontoActStr, CAST(ISNULL(V1.MontoDia, 0) AS FLOAT) as MontoAct, CAST(ISNULL(V2.MontoDia, 0) AS FLOAT) as MontoSem, CAST(ISNULL(V3.MontoDia, 0) AS FLOAT) as MontoMes, CAST(ISNULL(V1.UnidadesDia, 0) AS INT) as UniAct, CAST(ISNULL(V2.UnidadesDia, 0) AS INT) as UniSem, CAST(ISNULL(V3.UnidadesDia, 0) AS INT) as UniMes, CONVERT(VARCHAR, C.FechaAct, 126) as FechaIso
                FROM Calendario C LEFT JOIN VentasBase V1 ON V1.F = C.FechaAct LEFT JOIN VentasBase V2 ON V2.F = C.FechaSemAnt LEFT JOIN VentasBase V3 ON V3.F = C.FechaMesAnt ORDER BY C.Orden ASC`);
        res.json(result.recordset);
    } catch (err) { res.status(500).json({ error: err.message }); }
});
        ...
        const result = await pool.request()
            ...
            .query(`
                WITH ItemsLimpios AS (
                    SELECT A.IdRouter, B.CODITM, A.CODCMP,
                    ISNULL((SELECT TOP 1 VAL.DESCRIPCION FROM dbo.QRITEMSATRIB ATR INNER JOIN dbo.QRATRIBUTOSVAL VAL ON ATR.CODATR = VAL.CODATR AND ATR.CODATRVAL = VAL.CODATRVAL WHERE ATR.CODITM = B.CODITM AND ATR.CODATR = 'R'), 'SIN RUBRO') AS RubroNombre,
                    B.CANTIDAD1 as CantidadFila
                    FROM dbo.QRMVS A INNER JOIN dbo.QRLINEASITEMS B ON A.IdRouter = B.IdRouter
                    WHERE CAST(A.CODSUC AS VARCHAR) LIKE '%' + @suc AND CAST(A.FECHA AS DATE) BETWEEN @desde AND @hasta AND A.ANULADO = 0 AND B.IDCOMPROBANTE > 0 AND A.CODCMP IN ('FA', 'FB', 'CA', 'CB') AND B.CODITM NOT LIKE 'DC%'
                )
                SELECT RubroNombre AS Rubro, CAST(SUM(CASE WHEN CODCMP IN ('CA', 'CB') THEN -CantidadFila ELSE CantidadFila END) AS INT) AS TotalUnidades
                FROM ItemsLimpios GROUP BY RubroNombre ORDER BY TotalUnidades DESC`);
        res.json(result.recordset);
    } catch (err) { res.status(500).json({ error: err.message }); }
});
        ...
        const result = await pool.request()
            ...
            .query(`
                WITH VentasYDescuentos AS (
                    ...
                    FROM dbo.QRMVS A INNER JOIN dbo.QRLINEASITEMS B ON A.IdRouter = B.IdRouter
                    WHERE CAST(A.CODSUC AS VARCHAR) LIKE '%' + @suc AND CAST(A.FECHA AS DATE) BETWEEN @desde AND @hasta AND A.ANULADO = 0 AND B.IDCOMPROBANTE > 0
                ),
                GlobalBruto AS (SELECT SUM(ImporteNeto) as VentaBrutaTotal FROM VentasYDescuentos WHERE TipoFila = 'PRODUCTO')
                SELECT 'RESUMEN' as TipoFila, 'TOTAL' as Concepto, '' as Codigo, CAST(ISNULL((SELECT VentaBrutaTotal FROM GlobalBruto), 0) AS MONEY) as ValBruto, CAST(SUM(ABS(ImporteNeto)) AS MONEY) as ValDcto, CAST(ISNULL((SELECT VentaBrutaTotal FROM GlobalBruto), 0) - SUM(ABS(ImporteNeto)) AS MONEY) as ValNeto, CAST((SUM(ABS(ImporteNeto)) / NULLIF((SELECT VentaBrutaTotal FROM GlobalBruto), 0)) * 100 AS DECIMAL(10,2)) as Porcentaje, COUNT(DISTINCT IdRouter) as CantTickets FROM VentasYDescuentos WHERE TipoFila = 'DESCUENTO'
                UNION ALL SELECT 'DETALLE', DescripcionItem, CODITM, 0, CAST(SUM(ABS(ImporteNeto)) AS MONEY), 0, CAST((SUM(ABS(ImporteNeto)) / NULLIF((SELECT VentaBrutaTotal FROM GlobalBruto), 0)) * 100 AS DECIMAL(10,2)), COUNT(DISTINCT IdRouter) FROM VentasYDescuentos WHERE TipoFila = 'DESCUENTO' GROUP BY CODITM, DescripcionItem`);
...
app.get('/api/reporte/ventas-grilla-diaria', async (req, res) => {
    ...
    try {
        ...
        const result = await pool.request().input('desde', sql.Date, desde).input('hasta', sql.Date, hasta).query(`
                ...
                SELECT @cols = STUFF((SELECT ',' + QUOTENAME(CONVERT(VARCHAR, Fecha, 103)) FROM (SELECT DISTINCT CAST(FECHA AS DATE) as Fecha FROM dbo.QRMVS WHERE CAST(FECHA AS DATE) BETWEEN @desde AND @hasta) AS Dias ORDER BY Fecha FOR XML PATH(''), TYPE).value('.', 'NVARCHAR(MAX)'), 1, 1, '');
                SELECT @sum_cols = STUFF((SELECT '+ISNULL(CAST(' + QUOTENAME(CONVERT(VARCHAR, Fecha, 103)) + ' AS MONEY),0)' FROM (SELECT DISTINCT CAST(FECHA AS DATE) as Fecha FROM dbo.QRMVS WHERE CAST(FECHA AS DATE) BETWEEN @desde AND @hasta) AS Dias ORDER BY Fecha FOR XML PATH(''), TYPE).value('.', 'NVARCHAR(MAX)'), 1, 1, '');
                SET @query = 'SELECT SUCURSAL, ' + @cols + ', (' + @sum_cols + ') AS TOTAL_SUCURSAL FROM (SELECT S.NOMBRE AS SUCURSAL, CONVERT(VARCHAR, CAST(A.FECHA AS DATE), 103) AS Dia, SUM(CAST(CASE WHEN A.CODCMP IN (''FA'', ''FB'') THEN B.IMPORTE WHEN A.CODCMP IN (''CA'', ''CB'') THEN -B.IMPORTE ELSE 0 END AS MONEY)) AS Neto FROM dbo.QRSUCURSALES S LEFT JOIN dbo.QRMVS A ON S.CODSUC = A.CODSUC AND CAST(A.FECHA AS DATE) BETWEEN ''' + CAST(@desde AS VARCHAR) + ''' AND ''' + CAST(@hasta AS VARCHAR) + ''' AND A.ANULADO = 0 AND A.CODCMP IN (''FA'', ''FB'', ''CA'', ''CB'') LEFT JOIN dbo.QRLINEASITEMS B ON A.IdRouter = B.IdRouter AND B.IDCOMPROBANTE > 0 WHERE S.CODEMP <> 1 AND S.CODSUC NOT IN (996, 997) GROUP BY S.NOMBRE, CAST(A.FECHA AS DATE)) x PIVOT (SUM(Neto) FOR Dia IN (' + @cols + ')) p ORDER BY TOTAL_SUCURSAL DESC';
...
app.post('/api/login', async (req, res) => {
    try {
        ...
        const result = await pool.request().input('usuario', sql.VarChar, usuario).query(`SELECT TOP 1 * FROM dbo.usuarios_reportes WHERE usuario = @usuario`);
        if (result.recordset.length === 0) return res.status(401).json({ error: 'Credenciales invalidas' });
        const u = result.recordset[0];
        const ok = await bcrypt.compare(password, u.password_hash);
        if (!ok) return res.status(401).json({ error: 'Credenciales invalidas' });
        const token = jwt.sign({ id: u.id, usuario: u.usuario, rol: u.rol, sucursal: u.sucursal }, process.env.JWT_SECRET, { expiresIn: '8h' });
        res.json({ token, user: { id: u.id, usuario: u.usuario, rol: u.rol, sucursal: u.sucursal, activo: !!u.activo } });
    } catch (err) { res.status(500).json({ error: err.message }); }
});

// (Rutas de Usuarios Crear/Editar/Listar se mantienen igual)

const PORT = process.env.PORT || 3000;
app.listen(PORT, () => console.log(`🚀 Mimo BI Activo`));